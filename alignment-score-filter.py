...
-s		--secondary		Tag of the second best score				["ZS"]
-m		--mate			Tag of the alignment score of the mate			["YS"]

[ output ]
-u 		--undefined		Place here the scores that didn't pass the threshold	[-]


WARNINGS: 
 * input must be sam (unformatted) format. Workaround: pipe it from samtools view 
 * input sam file must be sorted by read name 
